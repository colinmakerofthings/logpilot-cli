...
def test_cli_missing_file():
    result = run_cli(["not_a_real_file.log"])
...
def test_cli_out_file(tmp_path):
    ...
    result = run_cli([str(log_file), "--out-file", str(out_file)])
...
def test_cli_output_json(tmp_path):
    ...
    result = run_cli([str(log_file), "--output", "json"])
    assert result.returncode == 0, f"stdout: {result.stdout}\nstderr: {result.stderr}"
...
def test_cli_format_json(tmp_path):
    ...
    result = run_cli([str(log_file), "--format", "json"])
    assert result.returncode == 0, f"stdout: {result.stdout}\nstderr: {result.stderr}"
...
def test_cli_format_text(tmp_path):
    ...
    result = run_cli([str(log_file), "--format", "text"])
